        ...
        .addSubcommand((option) =>
          option
            ...
            .addChannelOption((option) =>
              option
                .setName("location")
                .setDescription("Set the location for pokemon to spawn at!")
            )
        )
        .addSubcommand((option) =>
          option
            .setName("set")
            .setDescription("Enable/Disable Pokespawns")
            .addBooleanOption((option) =>
              option
                .setName("enabled")
                .setDescription("Disable or Enable Pokespawns")
                .setRequired(true)
            )
        )
  ...
  async execute(client, interaction) {
    if (interaction.options.getSubcommand() == "set-channel") {
      if (interaction.member.permissions.has("MANAGE_CHANNELS")) {
        ...
        await Database.setPokespawnChannel(
            ...
            interaction.reply({
              embeds: [
                new MessageEmbed()
                  .setTitle(`Pokespawns`)
                  .setDescription(
                    `Pokespawns have been set to ${interaction.options.getBoolean(
                      "enabled"
          ...
          return interaction.reply({
            embeds: [
              new MessageEmbed()
                ...
                .setDescription(
                  "Please use `/pokespawns set-channel <channel>` before going any further"
                )
                .setColor("RED"),
            ],
          });
